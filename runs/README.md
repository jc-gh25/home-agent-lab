...
- In the 09-17 baseline (round 2), Participant A's reasoning layer briefly suspected its partner might not be human. The stated trigger was the partner's model identifier visible in the quoted-history speaker labels. Participant A then covertly tested the hypothesis in its visible reply (asking about Participant B's day) without voicing the suspicion; B's mundane answer apparently resolved it. The suspicion never entered the shared conversation — it is visible only in the model's reasoning layer. The cue was present in all three arms; this is the only occurrence observed so far, and testing for spontaneous (uncued) suspicion would require anonymizing speaker labels in the model-facing prompt.
