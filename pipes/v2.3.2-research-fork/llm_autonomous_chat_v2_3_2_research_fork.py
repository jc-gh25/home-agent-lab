...
CHANGELOG
================================================================================
v2.3.2 Sidecar repair.

- Restores the per-turn "turn" sidecar event, accidentally dropped from
  the v2.3.0 rewrite of append_to_log. v2.3.0 and v2.3.1 wrote full turn
  metrics to the .txt log but emitted NO turn events to the .jsonl
  sidecar (run_start, config_check, model_preflight, run_aborted,
  loop_detected, and run_end were unaffected). Found in a v2.3.1 2-round
  smoke test on 2026-09-21 (run tag topic-anchor-v2.3.1-on-001-preflight-a):
  run_end arrived 116 s after preflight with zero turn events while the
  .txt contained both rounds.
- The restored turn event now also carries the v2.3 fields
  note_parse_status, generation_status, and turn_validity.
- The fail-fast run_aborted event now includes raw_content and
  raw_reasoning, not just their character counts.
...
class Pipe:
    ...
    PIPE_VERSION = "2.3.2 (research fork)"
    ...
    def append_to_log(
        ...
        # v2.3.2: sidecar event with full structured detail. This block was
        # accidentally dropped from the v2.3.0 rewrite of append_to_log;
        # v2.3.0 and v2.3.1 wrote turn metrics to the .txt log only, so no
        # "turn" events reached the .jsonl sidecar. Restored from v2.2.0 and
        # extended with the v2.3 fields note_parse_status,
        # generation_status, and turn_validity.
        self._sidecar_write(
            {
                "event": "turn",
                "round": round_number,
                ...
                "note_parse_status": metrics.get("note_parse_status"),
                ...
                "turn_validity": metrics.get("turn_validity"),
                        ...
                        self._sidecar_write(
                            {
                                ...
                                # v2.3.2: preserve the raw output itself, not
                                # just its character counts. The abort status
                                # message already claimed this was preserved.
