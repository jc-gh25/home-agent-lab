...
version: 2.0.0 (research fork of the v1.9.0 sandbox fork)
tested_open_webui_version: 0.11.3
...
class Pipe:
    ...
    PIPE_VERSION = "2.0.0 (research fork)"
    ...
    def _run_header_lines(self, original_prompt: str, is_continuation: bool) -> List[str]:
        """Every valve that can affect behavior, recorded at run start."""
        v = self.valves
        lines = [
            ...
            "NOTE: Server-side settings (LM Studio context length and overflow policy,",
            "quantization, GPU config) are NOT captured automatically. Record them in",
            "LM Studio and match them to this header when publishing.",
        ]
        return lines
    ...
    def init_log_file(
        ...
        try:
            ...
            # Sidecar: run-start event with the same provenance.
            self._sidecar_write(
                {
                    "event": "run_start",
    ...
    def run_turn(
        ...
        metrics["elapsed_seconds"] = round(time.monotonic() - turn_start, 2)
    ...
    def pipe(
        ...
        if not self.valves.MODEL_A.strip() or not self.valves.MODEL_B.strip():
            yield "Configure MODEL_A and MODEL_B in this function's Valves before using this pipe."
            return
        ...
        try:
            for offset in range(self.valves.MAX_ROUNDS):
                ...
                for participant, model, system_prompt, other_id in (
                    ...
                    result: Dict[str, Any] = {"content": "", "reasoning": "", "metrics": {}}
            ...
            if moderator_model:
                ...
                if trimmed:
                    notes += "[Earlier discussion was removed because of context limits.]\n"
